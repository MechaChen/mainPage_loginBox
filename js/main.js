...
function moveScene(e) {
  ...
  if (y > innerHeight - SCROLLAREA.height) {
    const currentRatio = 1 - (innerHeight + SCROLLAREA.height - y) / SCROLLAREA.height;
    const speed = (SCROLLAREA.maxSpeed * currentRatio);
    SCROLLAREA.currentSpeed = (speed < SCROLLAREA.minSpeed) ? SCROLLAREA.minSpeed : speed;
  ...
  let height_nav = document.querySelector('.common_header').offsetHeight;
  let browserHeight = 100;
  if (y < SCROLLAREA.height + height_nav + browserHeight) {
    // console.log(SCROLLAREA.height + height_nav, y);
    const currentRatio = (SCROLLAREA.height + height_nav + browserHeight - y) / (SCROLLAREA.height + height_nav + browserHeight); //比例
    const speed = (SCROLLAREA.maxSpeed * currentRatio);
    SCROLLAREA.currentSpeed = (speed < SCROLLAREA.minSpeed) ? SCROLLAREA.minSpeed : speed;
...
function init() {
  ...
  window.addEventListener("resize", function(){
    if (innerWidth / innerHeight > 1.75) {
      ...
      htmlSize[0] = document.documentElement.clientHeight;
...
function initWebsocketServer() {
  conn_chat = new WebSocket('ws://localhost:8080/chat');
  conn_chat.onopen = function (e) {
    console.log('已連到伺服器');
    // temp solution
    if (typeof (user_id) == "undefined") {
      var user_id = prompt("input your nickname");
      login(user_id);
    }
  };
